...
data class Team (val players: List<Player>, val name: TeamName) {
    ...
    val hasEnoughStartingPlayers : Boolean
        get() = players.filter { player -> player.isStarting }.size >= 15
    ...
    fun replacingCaptainBackNumber(): Int? {
        return scrumhalf()?.backNumber ?: players.first { player -> player.isStarting }.backNumber
    }

    fun scrumhalf() : Player? {
        return players.find { player -> player.backNumber == 7 }
    }
}
